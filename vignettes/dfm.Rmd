...
```
As previously mentioned, small values can result in inaccurate estimations. It's therefore a good idea to scale up the data; this is done by default but can be turned off by setting `scale = FALSE`.

[TODO: The package should take care of technical stuff, not the user. If scaling is a good idea, we should do it by default, and unscale at the end. With an option to turn it of => This isn't so obvious to deal with. What I've done is put together very simple `scale()` and `unscale()` functions to make this step as easy as possible. DONE]
