...
```r
Est <- dfm(Y, factors = 1, lags = 2, forecast = 0, B_prior = NULL, lam_B = 0, H_prior = NULL, lam_H = 0, nu_q = 0, nu_r = NULL, ID = 'PC_full', intercept = TRUE, reps = 1000, burn = 500)
```
...
```dates```   If input is `ts_boxable()`, dates corresponding to indexes of data
...
Examples scripts are included in the [Examples](https://github.com/srlanalytics/bdfm/tree/master/inst/Examples) file of this repository including code to run your own live nowcasts of US GDP using the [Fred](https://fred.stlouisfed.org/) API:

![](https://github.com/srlanalytics/bdfm/blob/master/img/US_GDP.png)
